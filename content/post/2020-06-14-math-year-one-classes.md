...
numbers: \
for example  *4x + 5 = 0* solves to *x=-5/4*. \
...
*√2*, to the rationals, we expect the rules of arithmetic to remain the same.
...
equation, we have added √2 to the rationals. It turns out that if *a + b √2* is
a solution of our equation, then so is *a - b √2* (for equations with real
coefficients). \
The mapping *a+b√2 → a-b√2* is a transformation that maps the
...
In calculus you used *ε  - 𝛿* ("epsilon-delta") arguments, all about a
*neighborhood* of points around a point *x* that are less than *𝛿* away from
it. In topology you cut straight to the set of points "near" *x* and call them
